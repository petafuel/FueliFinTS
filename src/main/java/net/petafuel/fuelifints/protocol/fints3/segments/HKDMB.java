...
public class HKDMB extends Segment implements IExecutableElement {
    ...
    @Element(
            description = {@ElementDescription(number = 5, status = ElementDescription.StatusCode.C, length = -4)})
    @num
    private Integer maximaleAnzahlAuftraege;

    @Element(
            description = {@ElementDescription(number = 6, status = ElementDescription.StatusCode.C, length = -35)})
    private String aufsetzpunkt;

    public HKDMB(byte[] message) {
        super(message);
    }
    ...
    public StatusCode execute(Dialog dialog) {
        ...
        List<DirectDebitDataObject> lastschriftBestand = dataAccessFacade.getSammelLastschriftBestand(dialog.getLegitimationsInfo());
        if (lastschriftBestand == null || lastschriftBestand.size() == 0) {
			try {
				statusElement.addRueckmeldung(new Rueckmeldung("3010::Es liegen keine Eintraege vor".getBytes("ISO-8859-1")));
