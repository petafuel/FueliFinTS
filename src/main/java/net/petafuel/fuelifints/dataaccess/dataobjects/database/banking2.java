...
public class banking2 {
    ...
    private String insertTempclientstaff(String dialogId, LegitimationInfo legitimationInfo, ClientProductInfo clientProductInfo) {
        ...
        try {
            ...
            LOG.warn(addTagUser(format("Fehler beim anlegen einer KundenSystemID: %s", e.getMessage()), legitimationInfo), e);
        } finally {
            ...
            if (preIban != null) {
                for (String iban : kontenListe.keySet()) {
                    if (!iban.equals(preIban)) {
                        ...
                        r = new ReturnDataObject(false, format("Kontoverbindung (%s) ist für dieses Auftragskonto nicht zugelassen", iban), "9210");
                        break;
                    }
                }
            } else {
        ...
        private static final class login {
            ...
            private static final class konten {
                private static final class permission {
                    ...
                    private static final String select = "SELECT IF(lkp.ueberweisung = 1, 1, IF(lkp.ueberweisung_ref = 1, 1, 0)) AS ueberweisung, lkp.sdd AS sdd, lkp.ade AS ade, lkp.azv AS azv, IF(lkp.lastschrift = 1, 1, IF(lkp.lastschrift_ref = 1, 1, 0)) as lastschrift FROM login_konten_permission lkp, konten k WHERE k.kontoid=lkp.kontoid AND lkp.benutzerkennung=? AND k.kontonummer=?";    //s
                    ...
                    private static final String calc_checksum = "SELECT sum(IF(lkp.ueberweisung = 1, 1, IF(lkp.ueberweisung_ref = 1, 1, 0))+(IF(lkp.lastschrift = 1, 1, IF(lkp.lastschrift_ref = 1, 1, 0))*10)+(abbuchung*100)+(dtaus*1000)+(sdd*10000)) AS checksum FROM login_konten_permission WHERE benutzerkennung=?";
                }
            }
        }
